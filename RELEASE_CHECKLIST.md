# Zimbra-QBO Billing v1.12.1 Production Release Checklist
...
- [ ] Create release tag:
  ```bash
  git tag -a v1.12.1 -m "Production release - secure, idempotent billing"
...
- [ ] Backup current database:
  ...
  cp data/billing.db /path/to/backups/billing_pre_v1.12.1.db
  ```
- [ ] Verify backup is readable:
  ```bash
  sqlite3 /path/to/backups/billing_pre_v1.12.1.db "SELECT COUNT(*) FROM customers;"
  ```
...
- [ ] Tag production deployment:
  ```bash
  git tag -a v1.12.1-prod-deployed -m "Deployed to production $(date +%Y-%m-%d)"
  git push origin v1.12.1-prod-deployed
  ```
